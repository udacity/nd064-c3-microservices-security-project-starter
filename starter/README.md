...
* Using the provided Vagrant file, deploy open-suse/Leap-15.2.x86_64 using vagrant file in the starter repo. Install all dependencies. Reminder: go dependences and linux-bench needs to be built and installed from source as per the exercise.
...
* Deploy the hardened opensuse linux base image to create a docker file harden using [docker -bench](https://github.com/aquasecurity/docker-bench).
* Create a docker file using the hardened image. Install all dependencies. Reminder: go dependences and docker-bench needs to be built and installed from source as per the exercise.
...
* Commit the hardend docker image to a private docker registry you created during the exercise.
